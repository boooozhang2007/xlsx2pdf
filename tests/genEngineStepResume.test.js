...
test('test papers honor the sampled word pool and cover a primary-school 20-word group', async () => {
  ...
  assert.equal((xml.match(/（20题）/g) || []).length, 6)
  assert.match(xml, /六、Synonym Matching 同义词匹配（4组20词，每组5词）/)
  assert.match(xml, /七、Antonym Matching 反义词匹配（4组20词，每组5词）/)
  assert.equal((xml.match(/第 4 组/g) || []).length, 2)
  assert.doesNotMatch(xml, /f\. (?:similar|opposite)/)
})
