...
VIVI_LLM_REQUEST_RETRIES=可选，单个 Workflow step 内的请求次数，默认 1；短暂失败由 Workflow 保存进度后续跑
VIVI_LLM_REQUEST_MAX_REQUEUES=可选，网络超时或服务端错误的自动续跑上限，默认 8
...
GEN_QUEUE_LLM_BATCH_ROUNDS_PER_STEP=可选，每个 Workflow step 最多连续执行的 LLM 请求轮数，默认 8；任务降级到小批次/低并发时会自动缩小单步工作量，避免超时后等待失联接管
